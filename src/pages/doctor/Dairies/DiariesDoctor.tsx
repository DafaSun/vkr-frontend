...
const DairiesDoctor = () => {
    ...
    const SearchClick = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:8000/api/doctor/diaries/?surname=${surname}`);
                ...
                <div className={styles['main-container']}>
                    <div className={self_styles['filters-container']}>
                        <div className={self_styles['row-container']}>
                            ...
                            <Button color={'blue'} text={'Применить'} onClick={SearchClick}/>
                        </div>
                    </div>
                            ...
                            guests.map(guest => (
                                <div className={self_styles['people-container']}>
                                    ...
                                    <Button color={'green'} text={'Перейти'}
                                             onClick={() => navigate(`/doctor/diaries/person?id=${guest.guest_id}`)}/>
                                </div>
                            ))
                        ) : (
